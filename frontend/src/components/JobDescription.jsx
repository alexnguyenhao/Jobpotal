...
const InfoBox = ({ icon, label, value, className = "" }) => (
  <div className={`flex items-center gap-3 ${className}`}>
    ...
    <div>
      <p className="text-xs text-white/70 md:text-gray-500 font-medium mb-0.5">{label}</p>
      <p className="font-bold text-white md:text-gray-900 text-sm md:text-base line-clamp-1">{value}</p>
    </div>
  </div>
);
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] pb-20">
        ...
        <div className="max-w-6xl mx-auto">
             <div className="flex flex-col md:flex-row gap-6 items-start">
                ...
                <div className="flex-1 w-full">
                    ...
                    {/* ... (Các thông tin công ty, deadline...) ... */}
                     <div className="flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-6 mb-6">
                         ...
                         <span className="text-gray-500 text-sm flex items-center gap-1 bg-gray-50 px-2 py-1 rounded">
                            <Clock size={14} /> Application deadline: {new Date(applicationDeadline).toLocaleDateString("en-GB")}
                         </span>
                      </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto mt-2">
                        <Button
                            ...
                            disabled={isApplied || isExpired || isApplying} // Thêm disabled khi đang apply
                            onClick={() => !isApplied && !isExpired && (user ? setOpenResumeDialog(true) : navigate("/login"))}
                        >
                            <Share2 className="w-4 h-4 mr-2" />
                            {isApplying ? "Applying..." : isApplied ? "Applied Successfully" : isExpired ? "Job Closed" : "Apply Now"}
                        </Button>

                        <Button
                            variant="outline"
                            className={`h-11 px-6 rounded-lg border-gray-300 font-semibold flex-1 md:flex-none hover:bg-purple-50 hover:text-[#6A38C2] hover:border-purple-200 transition-all ${isSaved ? "text-red-500 border-red-200 bg-red-50" : "text-gray-700"}`}
                            onClick={handleSaveToggle}
                        >
                            <Heart size={18} className={`mr-2 ${isSaved ? "fill-red-500" : ""}`} />
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-0 grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* ... (Phần nội dung hiển thị Job Description không thay đổi) ... */}
              ...
              <Card className="border-none shadow-sm rounded-xl overflow-hidden">
                 <CardContent className="p-6 md:p-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                    {benefits?.map((ben, i) => (
                      <div key={i} className="flex items-center gap-2 p-3 bg-purple-50/50 rounded-lg border border-purple-100 hover:border-purple-200 transition-colors">
                        <div className="w-1.5 h-1.5 rounded-full bg-[#6A38C2]" />
                        <span className="text-gray-800 font-medium text-sm">{ben}</span>
                      </div>
                    ))}
                  </div>
                </CardContent>
        ...
        <div className="lg:col-span-4 space-y-6">
           ...
           <Card className="border-none shadow-sm rounded-xl bg-white top-24">
            <CardContent className="p-6">
              <h4 className="font-bold text-gray-900 mb-5 text-lg">General Information</h4>
              ...
              <div className="space-y-5">
                ...
                <GeneralInfoItem 
                  icon={<Users size={20} />}
                  label="Positions"
                  value={`${singleJob.position || 1} openings`}
                />
                <Separator />
                ...
                <GeneralInfoItem 
                   ...
                   label="Days Left"
                   value={
                     <span className={daysLeft <= 3 ? "text-red-500 font-bold" : "text-green-600 font-bold"}>
          ...
          <Card className="border-none shadow-sm rounded-xl bg-white">
            <CardContent className="p-6">
               ...
               <div className="flex items-center gap-4 mb-4">
                  ...
                  <div className="flex-1 overflow-hidden">
                     ...
                     <a href={company?.website} target="_blank" className="text-xs text-gray-500 hover:text-[#6A38C2] flex items-center gap-1 mt-1">
               ...
               {singleJob.company.location && (
                  <div className="text-sm text-gray-600 flex gap-2 mb-4">
                    ...
                    <span className="line-clamp-2">{singleJob.company.location}</span>
                  </div>
               )}
           ...
           <div className="bg-[#E6E0F8] rounded-xl p-6 text-center">
              ...
              <p className="text-sm text-gray-600 mb-4">Share this opportunity with someone who might be interested.</p>
              <Button 
